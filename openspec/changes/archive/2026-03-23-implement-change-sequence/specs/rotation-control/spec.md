...
#### Scenario: 执行旋转
- **WHEN** 用户启动旋转任务
- **THEN** 系统按配置的次数自动旋转，每次旋转后等待机械轴就绪信号
...
### Requirement: 自动模式控制
系统 SHALL 使用PLC自动模式1（间隔运行+触发）实现旋转控制。
...
#### Scenario: 监控进度
- **WHEN** 旋转进行中
- **THEN** 监控S407（coilReady）检测每次移动完成
...
- **THEN** 系统执行12次旋转，每次30°，速度10°/s，触发模式控制
...
#### Scenario: 异常自动处理
- **WHEN** 后台错误检测线程发现PLC异常
- **THEN** 根据配置的异常处理模式执行复位或继续操作